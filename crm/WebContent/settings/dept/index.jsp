...
<script type="text/javascript">
	...
	$(function(){
		$("#saveBtn").click(function(){
			...
			$.ajax({
				type : "post",
				url : "${pageContext.request.contextPath}/settings/dept/save.do",
				data : {
					"no" : $.trim($("#no").val()),
					"name" : $.trim($("#name").val()),
				...
				beforeSend : function(){
					...
					return false;
					*/
					// 验证部门编号不能为空，并且必须是4位数字（实际上还需要验证唯一性。）
					var no = $("#no").val();
					if(no == ""){
						alert("部门编号不能为空！");
						return false;
						...
						var regExp = /^[0-9]{4}$/;
						var ok = regExp.test(no);
						if(ok){
							return true;
						}else{
							alert("部门编号必须是4个长度的数字！");
							return false;
						}
					}
				},
		...
		$("#popCreateDeptModalBtn").click(function(){
			// 清空表单中的所有数据（重置表单）
			// $("#deptForm").reset(); // 重点：jquery对象没有reset()方法。reset()方法属于dom对象。
			$("#deptForm")[0].reset();
			// 显示modal窗口
				...
				<div class="modal-body">
					...
					<form id="deptForm" class="form-horizontal" role="form">
						...
						<div class="form-group">
							...
							<div class="col-sm-10" style="width: 300px;">
								<input type="text" class="form-control" id="no" style="width: 200%;" placeholder="编号为四位数字，不能为空，具有唯一性">
							</div>
						</div>
	...
	<div style="width: 95%">
		...
		<div class="btn-toolbar" role="toolbar" style="background-color: #F7F7F7; height: 50px; position: relative;left: 30px; top:-30px;">
			<div class="btn-group" style="position: relative; top: 18%;">
			  <button type="button" class="btn btn-primary" id="popCreateDeptModalBtn"><span class="glyphicon glyphicon-plus"></span> 创建</button>
