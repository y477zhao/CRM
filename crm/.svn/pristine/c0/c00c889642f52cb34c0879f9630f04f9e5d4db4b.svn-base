...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#createBtn").click(function(){
			// 发送ajax get，查询所有的部门。
			$.get(
				"${pageContext.request.contextPath}/settings/qx/user/create.do",
				// {"_" : new Date().getTime()},
				"_=" + new Date().getTime(), // 这里没有cache : false
				function(array){
					// 动态生成部门下拉列表
					// [{"no":"","name":""},{"no":"","name":""}]
		...
		$("#saveBtn").click(function(){
			// 发送ajax post请求。（表单验证）
			$.post(
				"${pageContext.request.contextPath}/settings/qx/user/save.do",
				{
